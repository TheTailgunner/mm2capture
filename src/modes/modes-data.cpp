...
QByteArray
ModesData::getMessage(MessageType type) const {
    Q_ASSERT(type != MessageType::None);
...
unsigned
ModesData::getBeastFrameLength(quint8 typeByte) {
    switch (typeByte) {
    case 0x31:
        return 2;
    case 0x32:
...
QByteArray
ModesData::getMessageBeast() const {
    ...
    res[0] = 0x1A;
    res[1] = typeByte;
    std::memcpy(reinterpret_cast<void*>(res.data() + 2),
            m_beastMlatTimestamp, MLAT_LEN_BYTES);
    res[8] = m_beastSignalLevel;
    std::memcpy(reinterpret_cast<void*>(res.data() + 9),
